...
- 但是这是一种"has a"即组合的关系，外层结构体拥有一个内层结构体
...
distanceFromP := p.Distance  // distanceFromP的调用看起来和函数调用没有区别
fmt.Println(distanceFromP(q))  // 但是却存在一个隐形的实参，即方法的接收者
```

#### 方法表达式

上面的方法值拥有一个默认的实参（接收者），而方法表达式把接收者放在了参数列表第一个的位置，从而对很多不同的接收者调用该方法：
